...
/*			strncpy(tmp_charge, &line[60], 8);
			...
			if ( current_conformer == 0) {
				...
				if( res_type[0] == 'W' && res_type[1] == 'A' && res_type[2] == 'T')
					++wats;
				...
				{
					fprintf(stderr,"%s is metal",mols->atom_names[current_atom]);
				}
 ...
 *
 */
int add_protein(MOL2 **mymol, char *finput_name)
{
	FILE *input = NULL;
	char *line = NULL;
	MOL2 *mols = NULL;
	...
	mols->pcharges = (float*)realloc(mols->pcharges, sizeof(float) * mols[i].n_atoms);
	mols->radius = (float*)realloc(mols->radius, sizeof(float) * mols[i].n_atoms);
	mols->atoms = (int*)realloc(mols->atoms, sizeof(int) * mols[i].n_atoms);
	mols->ringer = (int*)realloc(mols->ringer, sizeof(int) * mols[i].n_atoms);
	mols->aromatic = (int*)realloc(mols->aromatic, sizeof(int) * mols[i].n_atoms);
	mols->bond_dist = (float*)realloc(mols->bond_dist, sizeof(float) * mols[i].n_atoms);
	mols->grads_X = (float*)realloc(mols->grads_X, sizeof(float) * mols[i].n_atoms);
	mols->grads_Y = (float*)realloc(mols->grads_Y, sizeof(float) * mols[i].n_atoms);
	mols->grads_Z = (float*)realloc(mols->grads_Z, sizeof(float) * mols[i].n_atoms);
	mols->backbone = (int*)realloc(mols->backbone, sizeof(int) * mols[i].n_atoms);
        mols->exclude = (int*)calloc(sizeof(int), mols->n_atoms);
	mols->fragment_flag = (int*)realloc(mols->fragment_flag, sizeof(int) * mols[i].n_atoms);
	mols->selection = (int*)realloc(mols->selection, sizeof(int) * (mols[i].n_atoms + 10));
	mols->gaff_types = (int*)realloc(mols->gaff_types, sizeof(int) * (mols[i].n_atoms + 1));
		...
		if ( strstr(line, "ATOM") != NULL || strstr(line, "HETATM") != NULL) {
			...
			if ( tmp_atom[0] == 'C' && ( tmp_atom[1] != 'l' && tmp_atom[1] != 'L'))
				...
				if ( tmp_atom[1] == 'H')
					...
					mols->atoms[current_atom] = 11;
					fprintf(stderr, "AddProtein %i: Error. Do not know the atom type %s\n", __LINE__,tmp_atom);
				}
			}
			strncpy(myx, &line[29], 10);
			strncpy(myy, &line[38], 10);
			...
			for (i = 0; i < 429; ++i) {
				if ( strstr(prot_names[i * 2], tmp_resname) != NULL) {
					...
					if ( match == strlen(tmp_atom) && match == strlen(prot_names[(i * 2) + 1]))
 ...
 *
 */
int MultiPDB_reader(MOL2 **mymol, char *finput_name, int import)
{
