...
char *argv[];
{
	...
	float MMenergy = 0, desolvEnergy = 0, totalEnergy = 0;
        ...
        /* Desolvation */
        /* Protein */
        int *numOverlaps_prot = NULL, *buried_prot = NULL, **OverlapsMatrix_prot = NULL;
        ...
        int *numOverlaps_hbond_prot = NULL, *buried_hbond_prot = NULL, **OverlapsMatrix_hbond_prot = NULL;
        float **AijMatrix_hbond_prot = NULL;
        int *numOverlaps_hbond = NULL, *buried_hbond = NULL, **OverlapsMatrix_hbond = NULL;
        ...
        int *numOverlapsLigComplex_hbond = NULL, *numOverlapsProtComplex_hbond = NULL;
	...
	int nres = 0, res_idx = 0, last_res = 0;
	float current_qq = 0.0f, current_vdw = 0.0f;
	float qq_average = 0.0f, vdw_average = 0.0f, dl_average = 0.0f, dr_average = 0.0f, apolar_average = 0.0f;
	float total_average = 0.0f;

	int i = 0, j = 0, k = 0, real_frames = 0, jj = 0, ii = 0, l = 0, ll = 0, kk = 0;
	...
	entropy_flag = 0;
	if( verbose_flag)
		fprintf(stderr,"This binary was not compiled with entropy calculation support\n");
	#endif
	...
	if( rst_file == NULL && (mdcrd_file == NULL && base_mdcrd == NULL ) && pdb_file == NULL)
	{
		fprintf(stderr,"No valid input file. Please provide a AMBER topology/coordinates pair or a pdb file\n");
                fflush(stderr);
                show_help();
                exit(-1);
	}
	...
	{
		fprintf(stderr,"Topology provided but missing coordinates. Provide a valid mdcrd or rst/crd file\n");
                fflush(stderr);
                show_help();
                exit(-1);
	}
	...
	if( pdb_mode == 0)
	{
		if( mdcrd_mode == 1)
		{
			...
			if( xtc_flag == 0)
			{
				...
				{
                                	fprintf(stderr,"Fatal error loading information from coordinate file\n");
                                	fflush(stderr);
                                	exit(-1);
				}
			}else{
			...
			if( entropy_flag == 1)
			{
				...
				for( i = 0; i < mol->n_atoms; i++)
				{	
					if( mol->atom_names[i][0] == 'C' && mol->atom_names[i][1] == 'A' && mol->atom_names[i][2] == ' ' && mol->res_num[i] < 2000)
					{
				...
				average_structure = (float **) calloc(sizeof(float *), entropy_natoms);
				reference_structure = (float **) calloc(sizeof(float *), entropy_natoms);
				for( i = 0; i < entropy_natoms; i++)
				{
					average_structure[i] = (float *) calloc(sizeof(float),3);
					reference_structure[i] = (float *) calloc(sizeof(float),3);
				}
				entropy_idx = 0;
                                for( i = 0; i < mol->n_atoms; i++)
                                {
				...
				quasi_matrix = (double *) calloc(sizeof(double), 3*entropy_natoms*3*entropy_natoms);


		                if( verbose_flag )
		                {
					fprintf(stderr,"Number of atoms involved in entropy calculation: %i\n",entropy_natoms);
					fflush(stderr);
				}
			}
...
/*	res_energies = (float *) calloc(sizeof(float),nres);*/
	...
	res_energies_details = (float **) calloc(sizeof(float*),last_res+1);
	/*res_averages = (float **) calloc(sizeof(float*),nres+1);*/
	res_averages = (float **) calloc(sizeof(float*),last_res+1);
	/*for( i = 0; i <= nres; ++i)*/
	for( i = 0; i <= last_res; ++i)
	{
	...
	*************************/
	...
	/* Initiallize vars for three layers */
        ...
        AijMatrix_hbond = (float **) calloc( sizeof( float *),ISM_MAX_OVERLAPS+1);
        OverlapsMatrix_hbond = (int **) calloc( sizeof( int *),ISM_MAX_OVERLAPS+1);
	...
	/* Free SASA Hard spheres no overlapping model */
        ...
        AijMatrixLig_hbond = (float **) calloc( sizeof( float *),ISM_MAX_OVERLAPS+1);
        AijMatrixProt_hbond = (float **) calloc( sizeof( float *),ISM_MAX_OVERLAPS+1);
        ...
        OverlapsMatrixLig_hbond = (int **) calloc( sizeof( int *),ISM_MAX_OVERLAPS+1);
        OverlapsMatrixProt_hbond = (int **) calloc( sizeof( int *),ISM_MAX_OVERLAPS+1);
        ...
        for( i = 0; i < ISM_MAX_OVERLAPS; i++)
        {
                OverlapsMatrixLig_hbond[i] = (int *) calloc(sizeof(int),lig->n_atoms);
                OverlapsMatrixProt_hbond[i] = (int *) calloc(sizeof(int),prot->n_atoms);
                AijMatrixLig_hbond[i] = (float *) calloc(sizeof(float),lig->n_atoms);
                AijMatrixProt_hbond[i] = (float *) calloc(sizeof(float),prot->n_atoms);
        }
        ...
        OverlapsMatrix_prot = (int **) calloc( sizeof( int *),ISM_MAX_OVERLAPS+1);
        ...
        AijMatrix_hbond_prot = (float **) calloc( sizeof( float *),ISM_MAX_OVERLAPS+1);
        OverlapsMatrix_hbond_prot = (int **) calloc( sizeof( int *),ISM_MAX_OVERLAPS+1);
        ...
        for( i = 0; i < ISM_MAX_OVERLAPS; i++)
        {
                ...
                OverlapsMatrix_hbond_prot[i] = (int *) calloc(sizeof(int),prot->n_atoms);
                AijMatrix_hbond_prot[i] = (float *) calloc(sizeof(float),prot->n_atoms);
	...
	tmp_file_name = (char *) calloc(sizeof(char), strlen(output_prefix)+40);
	...
	if( iframe <= 0)
        ...
        fprintf(f_energy_step,"Vdw;Colomb;DesolvLigand;DesolvReceptor;Apolar;Total\n");
	...
	if( xtc_flag != 0)
	{
        ...
        if( verbose_flag )
	{
		fprintf(stderr,"Requested Starting frame: %i | End frame: %i | Step size: %i\n",sframe,eframe,iframe);
        	fprintf(stderr,"--------------------------------------------------------\n");
	...
	for( i = sframe; i < eframe; i=i+iframe)
	{
		...
		if( ism_flag == 1)
		{
		        ...
		        ism_getInternalOverlaps_prot(prot,&numOverlaps_prot,&buried_prot,&AijMatrix_prot,&OverlapsMatrix_prot);
		        ism_get_free_sasa_prot(prot,total_surface_prot,buried_prot,numOverlaps_prot,AijMatrix_prot,
	                        OverlapsMatrix_prot,&lcpoComps_prot,&atomSASA_prot);
		        ism_get_distance_matrix(lig,&matrix);
		        ism_getInternalOverlaps(lig,matrix,&numOverlaps,&buried,&AijMatrix,&OverlapsMatrix);
		        ism_get_free_sasa(lig,total_surface,buried,numOverlaps,AijMatrix,
	                  OverlapsMatrix,&lcpoComps,&atomSASA);
		        ism_getExternalOverlaps(lig,prot,&buried_lig_complex, &buried_prot_complex,
	                  &numOverlapsLigComplex, &numOverlapsProtComplex, &OverlapsMatrixLig,
	                  &OverlapsMatrixProt, &AijMatrixLig, &AijMatrixProt);
		        ism_get_complex_sasa(lig,total_surface,buried,buried_lig_complex,
                             numOverlaps,numOverlapsLigComplex,numOverlaps_prot,numOverlapsProtComplex,
                             AijMatrix,AijMatrixLig,AijMatrix_prot,AijMatrixProt,
                             OverlapsMatrix,OverlapsMatrixLig,OverlapsMatrix_prot,OverlapsMatrixProt,
                             lcpoComps,&atomSASAComplex);
		        ism_get_complex_sasa_prot(prot,total_surface_prot,buried_prot,buried_prot_complex,
                             numOverlaps_prot,numOverlapsProtComplex,numOverlaps,numOverlapsLigComplex,
                             AijMatrix_prot,AijMatrixProt,AijMatrix,AijMatrixLig,
                             OverlapsMatrix_prot,OverlapsMatrixProt,OverlapsMatrix,OverlapsMatrixLig,
                             lcpoComps_prot,&atomSASAComplex_prot);
		        ism_getExternalOverlaps_forHbonds(lig,prot,&buried_lig_complex_hbond, &buried_prot_complex_hbond,
	                  &numOverlapsLigComplex_hbond, &numOverlapsProtComplex_hbond, &OverlapsMatrixLig_hbond,
	                  &OverlapsMatrixProt_hbond, &AijMatrixLig_hbond, &AijMatrixProt_hbond);
		        ism_get_complex_sasa_hbond(lig,total_surface,buried_hbond,buried_lig_complex_hbond,
                             numOverlaps_hbond,numOverlapsLigComplex_hbond,numOverlaps_hbond_prot,numOverlapsProtComplex_hbond,
                             AijMatrix_hbond,AijMatrixLig_hbond,AijMatrix_hbond_prot,AijMatrixProt_hbond,
                             OverlapsMatrix_hbond,OverlapsMatrixLig_hbond,OverlapsMatrix_hbond_prot,OverlapsMatrixProt_hbond,
                             lcpoComps,&atomSASAH);
		        ism_getExternalOverlaps_forHbonds(prot,lig,&buried_prot_complex_hbond, &buried_lig_complex_hbond,
	                  &numOverlapsProtComplex_hbond, &numOverlapsLigComplex_hbond, &OverlapsMatrixProt_hbond,
	                  &OverlapsMatrixLig_hbond, &AijMatrixProt_hbond, &AijMatrixLig_hbond);
		        ism_get_complex_sasa_hbond(prot,total_surface_prot,buried_hbond_prot,buried_prot_complex_hbond,
                             numOverlaps_hbond_prot,numOverlapsProtComplex_hbond,numOverlaps_hbond,numOverlapsLigComplex_hbond,
                             AijMatrix_hbond_prot,AijMatrixProt_hbond,AijMatrix_hbond,AijMatrixLig_hbond,
                             OverlapsMatrix_hbond_prot,OverlapsMatrixProt_hbond,OverlapsMatrix_hbond,OverlapsMatrixLig_hbond,
                             lcpoComps_prot,&atomSASAH_prot);
		        calculate_ga(lig,prot,&ga,&ga_prot,verbose_flag);
	
		        desolvR = ism_get_desolvation_energy(prot,lig,atomSASA_prot,atomSASAComplex_prot,atomSASAH_prot,ga_prot);
		        desolvL = ism_get_desolvation_energy(lig,lig,atomSASA,atomSASAComplex,atomSASAH,ga);
		        apolarcomplex = ism_get_apolar_energy(lig,prot,atomSASA,atomSASAComplex,total_surface,atomSASA_prot,atomSASAComplex_prot,total_surface_prot);
		...
		{
			if( pdb_mode == 0)
				MMenergy =  get_amber_vdw_selected(mol,topo,&res_energies_details,&current_vdw,&current_qq);
			else
				MMenergy = get_pdb_vdw_selected(mol,&res_energies_details,&current_vdw,&current_qq);

		}

		if( verbose_flag )
		{
	                fprintf(stderr,"vdW: %f | qq: %f | Desolvation (L/R/Apolar): %f %f %f\n",current_vdw,current_qq,desolvL,desolvR,apolarcomplex);
		}
		...
		if( mdcrd_mode == 1 || pdb_mode == 2)
		{
			...
			{
				entropy_idx = 0;
				fit_structure(&mol, reference_structure, entropy_selection, entropy_natoms);
		...
		for( j = 0; j < nres; j++)
		{
			res_energies_details[j][2] += desolvEnergy * ((res_energies_details[j][0]+res_energies_details[j][1])/MMenergy);
			res_averages[j][0] += res_energies_details[j][0]+res_energies_details[j][1]+res_energies_details[j][2];
			...
			fprintf(f_res_step,"%f;%f;%f;",res_energies_details[j][0],res_energies_details[j][1],res_energies_details[j][2]);
		...
		fprintf(f_energy_step,"%i,%f;%f;%f;%f;%f;%f\n",i+1,current_vdw,current_qq,desolvL,desolvR,apolarcomplex,totalEnergy);
		fflush(f_energy_step);
		...
		if( n_mdcrds > 0 && ( (i+iframe) >= eframe) && current_mdcrd < n_mdcrds)
		{
	...
	if( mdcrd_mode == 1 || pdb_mode == 2 )
	{
		...
		fprintf(f_res_average,"ResidueNumber,TotalEnergy,Vdw,qq,ISM\n");
			...
			fprintf(f_res_average,"%i,%f,%f,%f,%f\n",j+1,res_averages[j][0],res_averages[j][1],res_averages[j][2],res_averages[j][3]);
			fflush(f_res_average);
	        }
		...
		if( entropy_flag == 0)
		{
			fprintf(f_energy_average,"VDW;QQ;DesolvLigand;DesolvReceptor;Apolar;Total\n");
			fprintf(f_energy_average,"%f;%f;%f;%f;%f;%f\n",vdw_average,qq_average,dl_average,dr_average,apolar_average,total_average);
		}
	}
...
/*									printf("%i,%i %i,%i %f*%f = %f\n",entropy_idx,entropy_idx2,ll,kk,atom2_vec[kk],x1,atom2_vec[kk]*x1);
									fflush(stdout);*/
									quasi_matrix[l+kk] += atom2_vec[kk]*x1;
								  }
	  ...
	  if( entropy_flag == 1)
		...
		quasi_vector = (double *) calloc(sizeof(double),3*entropy_natoms*3*entropy_natoms);
		quasi_values = (double *) calloc(sizeof(double),3*entropy_natoms);

	        for( i = 0; i < 3*entropy_natoms; i++)
	        {
	                for( j = 0; j < 3*entropy_natoms; j++)
	                {
				quasi_matrix[(i*3*entropy_natoms)+j] /= (double) entropy_frames;
			}
		}
	        ...
	        for (j = 0; j < 3*entropy_natoms; j++)
	        {
		        ...
		        {
		            quasi_matrix[3*entropy_natoms*i+j] = quasi_matrix[3*entropy_natoms*j+i];
		        }
	        }
		...
		vl = vu = 0;
	        ...
	        LAPACKE_dsyevr( LAPACK_ROW_MAJOR, 'V', 'A', 'L', n, quasi_matrix, n, 0., 0.,
                           0, 0, abstol, &m, quasi_values, quasi_vector, 
                           n, isuppz );
		...
		qh_entropy = quasiharmonic_entropy(quasi_values,3*entropy_natoms);
                fprintf(f_energy_average,"VDW;QQ;DesolvLigand;DesolvReceptor;Apolar;Entropy;Total\n");
                fprintf(f_energy_average,"%f;%f;%f;%f;%f;%f;%f\n",vdw_average,qq_average,dl_average,dr_average,apolar_average,qh_entropy,total_average+qh_entropy);
	...
	if( mdcrd_mode == 1 || pdb_mode == 2)
	{
		...
		if( entropy_flag == 1)
		{
			fclose(f_eigenvals);
	                for( i = 0; i < entropy_natoms; i++)
	                {
				free(average_structure[i]); free(reference_structure[i]);
	                }
	...
	free(ga); free(ga_prot);
	free(buried); free(numOverlaps);
	free(atomSASA); free(atomSASAComplex); free(atomSASAH);
	free(total_surface);
	free(buried_hbond);
	...
	free(AijMatrix_hbond); free(OverlapsMatrix_hbond);
	free(AijMatrix); free(OverlapsMatrix);
	...
	free(AijMatrixLig); free(AijMatrixProt); free(OverlapsMatrixLig); free(OverlapsMatrixProt);
	free(OverlapsMatrixLig_hbond); free(OverlapsMatrixProt_hbond); free(AijMatrixLig_hbond); free(AijMatrixProt_hbond);
	free(OverlapsMatrix_prot); free(AijMatrix_hbond_prot); free(OverlapsMatrix_hbond_prot); free(AijMatrix_prot);
        free(buried_prot);
...
{
	fprintf(stderr,"");
        fprintf(stderr, "cMMISMSA - rev4 - Another program to quantify ligand-target interactions\n\n");
...
void show_help()
{
        ...
        fprintf(stderr,"Misc:\n");
        fprintf(stderr,"--mask <string> Residues to select as ligand. e.g. 1-6,10,20-30\n");
        ...
        fprintf(stderr,"--noentropy Disables Quasiharmonic approximation of entropy\n");
        fprintf(stderr,"--disable-ism Disables the Implicit Solvation Model\n");
        fprintf(stderr,"--disable-mm Disables the AMBER vdw and electrostatic terms\n");
