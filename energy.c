 ...
 *
 */
float get_chemscore_grid_energy_scaled(MOL2 *mol, MOL2 *prot,int *types, int flag)
{
	...
	float lp = 0.0f, hb = 0.0f, glp = 0.0f;
        ...
        if ( out_flag == 0) {
                ...
                for ( l = 0; l < real_atoms; ++l) {
                        kpot = types[l];
			lp = 0.0f;
                        if ( types[l] != -1 || mol->gaff_types[l] == SH ){
...
/*								energy += e1;*/
							...
							/* beta  */
							 ...
							 gbeta = 1.0f;
								...
								gbeta = gbeta * eval_block( fabs(180.0f-beta), 30.0f, 50.0f);
                                                        ...
                                                        e1 *= eval_block( fabs(180.0f-alfa), 30.0f, 50.0f);
							e1 *= gbeta;
							hb += e1;

							if (dist < 1.6)
							{
							  e1 = 20.0f*(1.6f-dist)/(3.34*1.6);
							  energy += e1;
							}
...
/*							fprintf(stderr,"End selection\n");
                                              ...
                                              if( prot->atoms[i] >= 100){
                                                ...
                                                if (dist > 3.0f)
                                                 continue;
                                                        ...
                                                        e1 = 8.0f*(eval_block( dist, 2.6f, 3.0f)*-6.03);
                                                fprintf(stderr,"Adding metal-thiol contribution: %f\n",e1);
                                                        energy += e1;

                                                if( dist < 1.3){
                                                        e1 = 20.0f*(1.3f-dist)/(6.03*1.3);
                                                }
					      ...
					      if( prot->atoms[i] >= 100){
							...
							e1 = (eval_block( dist, 2.6f, 3.0f)*-6.03);
                                                fprintf(stderr,"Adding metal contribution: %f\n",e1);
							if( mol->gaff_types[l] == SH)
							e1 *= 8.0f;
							energy += e1;
						...
						if( dist < 1.3){
							e1 = 20.0f*(1.3f-dist)/(6.03*1.3);
                                              ...
                                              }else if( (prot->gaff_types[i] == HN || prot->gaff_types[i]  == HO) )
                                              {
                                                        dx = x - prot->x[i];
                                                        dy = y - prot->y[i];
                                                        dz = z - prot->z[i];
...
/*                                                        fprintf(stderr,"Hydrogen bond A: %i %f %f %f %f\n",l,dist,alfa,gbeta,e1);*/
...
float get_desolv_penalization_fast(MOL2 *lig, MOL2 *prot, int *types, float *SASAligfree, float *SASAligcomplex, float *SASAprotfree, float *SASAprotcomplex, int *sel_lig, int *sel_prot)
{
	...
	{
		if( (types[i] == 2 || types[i] == 3) && sel_lig[i] == 0 ) /* Not in h-bond */
		{
			fraction = (SASAligfree[i] - SASAligcomplex[i])/SASAligfree[i];
			if( fraction > 0.50f && SASAligfree[i] > 0.0)
			{
			 penalization += 3.34; /* h-bond */
			 fprintf(stderr,"Penalizo atomo: %s%i ligando. %f %f %f\n",lig->atom_names[i],i,fraction,SASAligfree[i],SASAligcomplex[i]);
			}
		}
	}
		...
		if( ((prot->gaff_types[i] == O || prot->gaff_types[i] == OH || (prot->atoms[i] == 3 && has_hydrogens(prot,i) == 0 && prot->gaff_types[i] != N4)) || ( (prot->gaff_types[i] == HN || prot->gaff_types[i]  == HO) )) && sel_prot[i] == 0)
		{
			fraction = (SASAprotfree[i] - SASAprotcomplex[i])/SASAprotfree[i];
	
                        if( fraction > 0.50f && SASAprotfree[i] > 0.0)
			{
                         penalization += 1.67; /* h-bond */
                         fprintf(stderr,"Penalizo atomo: %s%i-%s proteina. %f %f %f \n",prot->res_names[i],prot->res_num[i],prot->atom_names[i],fraction,SASAprotfree[i],SASAprotcomplex[i]);
			}
		}
        }
...
float get_vdw_selected(MOL2 *lig, MOL2 *prot)
{
	float dx = 0.0f, dy = 0.0f, dz = 0.0f, dist = 0.0f;
	float dist2 = 0.0f, tmpE = 0.0f, vdwE = 0.0f;
	float distr = 0.0f, r6 = 0.0f, r12 =0.0f, epsilon = 0.0f;	
	float sigma6 = 0.0f;
	float cte = 0.0, lambda = 0.0f;
	float alpha = 1.0367f;
	float dScreening = 0.0f;
	float qqE = 0.0f;
...
/*                                tmpE = (332.0 * lig->pcharges[i] * prot->pcharges[j]) *  distr * distr ;
                        	...
                        	tmpE = 332.0 * lig->pcharges[i]*prot->pcharges[j]*distr/dScreening;
				
				qqE += tmpE;

                                epsilon = prevdw[ lig->gaff_types[i] - 1][prot->gaff_types[j] - 1 ][1];
                                sigma6 = prevdw[ lig->gaff_types[i] - 1][prot->gaff_types[j] - 1 ][2];

                                r6 = sigma6 / (dist2 * dist2 * dist2);
                                r12 = r6 * r6;

/*                                epsilon /= mol->vdw_type[j];*/
	...
	fprintf(stderr,"QQ: %f\n",qqE);
	fprintf(stderr,"VDW: %f\n",vdwE);
	return vdwE+qqE;
}

float get_vdw_intra(MOL2 *lig)
{
        float dx = 0.0f, dy = 0.0f, dz = 0.0f, dist = 0.0f;
        float dist2 = 0.0f, tmpE = 0.0f, vdwE = 0.0f;
        float distr = 0.0f, r6 = 0.0f, r12 =0.0f, epsilon = 0.0f;
        ...
        for( i = 0; i < lig->n_atoms; i++)
        {
                for ( j = i+1; j < lig->n_atoms; ++j) {
                                ...
                                epsilon = prevdw[ lig->gaff_types[i] - 1][lig->gaff_types[j] - 1 ][1];
                                sigma6 = prevdw[ lig->gaff_types[i] - 1][lig->gaff_types[j] - 1 ][2];
				...
				if( bonded(lig,i,j) == 1)
                                        epsilon = 0.0f;
                                else if(  q13_bond(lig, i,j) == 1)
                                        epsilon = 0.0f;
				else if( q14_bond(lig, i,j) == 1)
					epsilon /= 2.0f;

                                tmpE = epsilon * (r12 - 2.0 * r6);

/*				fprintf(stderr,"Pair: %i %i %f. FLAGS: %i %i %i\n",i,j,tmpE,q14_bond(lig, i,j),q13_bond(lig, i,j),bonded(lig,i,j));*/
                                vdwE = vdwE + tmpE;
