 ...
 *
 */
int transformate_mol(MOL2 **mymol, MOL2 **myprot, float *simplex, int nvars, int nrotors_prot, int **rotors, int **rotors_n, int **rotors_prot, int **rotors_n_prot)
{
 ...
 */
	for ( i = 0; i < real_atoms; ++i) {
        ...
        for( i = 0; i < nrotors; i++)
        {
/*		fprintf(stderr,"Best simplex Rotor %i: %f\n",i,simplex[i+6]);*/
/*                rot_tor(&mol, rotors_n[i][1], rotors_n[i][2], rotors_n[i][3], rotors_n[i][4], simplex[i+6]*180.f, rotors[i], rotors_n[i][0]);*/
        }

        for( i = 0; i < nrotors_prot; i++)
        {
/*                rot_tor(&prot, rotors_n_prot[i][1], rotors_n_prot[i][2], rotors_n_prot[i][3], rotors_n_prot[i][4], simplex[i+6+nrotors]*180.f, rotors_prot[i], rotors_n_prot[i][0]);*/
        }
...
/*                if ( mol->backbone[i] != 1)*/
                        printf("ATOM %6i %3s %3s  %4i    %8.3f%8.3f%8.3f  %1.2f %7.4f\n", i + 1, mol->atom_names[i], mol->res_names[i], mol->res_num[i], mol->x[i], mol->y[i], mol->z[i], mol->radius[i],mol->pcharges[i]);
...
int dump_pdb_conservative_file(MOL2 *mol,FILE *a)
{
        int i;
...
/*                if ( mol->backbone[i] != 1)*/
                        fprintf(a,"ATOM %6i %3s %3s  %4i    %8.3f%8.3f%8.3f  %1.2f %7.4f\n", i + 1, mol->atom_names[i], mol->res_names[i], mol->res_num[i], mol->x[i], mol->y[i], mol->z[i], mol->radius[i],mol->pcharges[i]);
...
int is_carboxylate(MOL2 *mol, int l)
{
        int i = 0, nh = -1, no = 0;
        int val = -1;
...
int is_hn_amido(MOL2 *mol, int l)
{
        int val = -1, i = 0, nh = 0;
        for(i = 0; i < mol->n_bonds; i++)
        {
